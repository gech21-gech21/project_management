...
import { MemberTaskInteraction } from "./task-detail-client";

export default async function MemberTaskDetailsPage({
  params,
}: {
  ...
  const task = await prisma.task.findUnique({
    where: {
      id: taskId,
      assignedToId: session.user.id, // Ensure member only sees their own tasks
    },
    include: {
      project: {
        select: {
